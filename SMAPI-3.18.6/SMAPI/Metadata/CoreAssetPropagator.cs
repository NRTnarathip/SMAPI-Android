...
namespace StardewModdingAPI.Metadata
{
    ...
    internal class CoreAssetPropagator
    {
            ...
            switch (assetName.BaseName.ToLower().Replace("\\", "/")) // normalized key so we can compare statically
            {
                ...
                ****/
                ...
                case "loosesprites/cursors": // Game1.LoadContent
                    ...
                    {
                        //Monitor.Log("day time money meu: " + menu.position);
                        foreach (ClickableTextureComponent button in new[] { menu.questButton, menu.zoomInButton, menu.zoomOutButton })
                        {
                            ...
                            //sometime button name is empty, null
                            //Monitor.Log("try set button >> " + button.label + " with mouse corsour >> " + Game1.mouseCursors.Name);
                ...
                ****/
                ...
                case "minigames/titlebuttons": // TitleMenu {
                    {
                        //Monitor.Log($"changed {changed}: update title button: content: {content}, assetName{assetName}");
        ...
        ****/
        ...
        private readonly struct LocationInfo(GameLocation location, Building parentBuilding) : IEquatable<LocationInfo>
        {
            public readonly GameLocation Location = location;
            public readonly Building ParentBuilding = parentBuilding;
            readonly bool IEquatable<LocationInfo>.Equals(LocationInfo other)
            {
                return Location == other.Location && ParentBuilding == other.ParentBuilding;
            }
        }
    }
}
