...
namespace StardewModdingAPI.Framework
{
    ...
    public class SCore : IDisposable
    {
        ...
        ****/
        ...
        public CommandQueue RawCommandQueue = new();
        ...
        *********/
        ...
        public static SCore Instance { get; private set; } = null!; // initialized in constructor, which happens before other code can access it
        ...
        *********/
        ...
        public SCore(string modsPath, bool writeToConsole, bool? developerMode)
        {
            ...
            {
                AndroidLog.Log("Error Try to init SCORE: " + ex.Message);
                AndroidLog.Log("Stacks: " + ex.InnerException);
            }
        ...
        public void RunInteractively()
        {
            ...
            try
            {
                ...
                // must be static since the game accesses it before the SGame constructor is called
                SGame.CreateContentManagerImpl = this.CreateContentManager;
                this.Monitor.Log("set func: CreateContentManagerImpl" + SGame.CreateContentManagerImpl);
                ...
                //set Object Game instance with SMAPI SGame
                GameRunner.instance = (GameRunner)this.Game;
        ...
        public void Dispose(bool isError)
        {
            ...
            this.Monitor.Log("Try to dispose SCore");
        ...
        *********/
        ...
        private void InitializeBeforeFirstAssetLoaded()
        {
            ...
            this.Monitor.Log("InitializeBeforeFirstAssetLoaded", LogLevel.Debug);
            // init TMX support
            ...
            try
            {
                ...
                *********/
                if (this.RawCommandQueue.TryDequeue(out string[]? rawCommands))
                {
                    ...
                    {
                        AndroidLog.Log("found raw cmd input: " + rawInput);
                        // parse command
                        string? name;
                        string[]? args;
                        Command? command;
                        ...
                        this.ScreenCommandQueue.GetValueForScreen(screenId).Add(new QueuedCommand(command, name, args));
                    }
                }
            ...
            try
            {
                ...
                *********/
                ...
                if (Context.IsSaving)
                {
                    ...
                    {
                        Monitor.Log("ERROR Try Saving: " + ex);
                    }
                    return;
                }
        ...
        {
#if !SMAPI_FOR_WINDOWS
            //Update Window title not support current plateform
            return;
#endif
        ...
        private bool TryLoadMod(IModMetadata mod, IModMetadata[] mods, AssemblyLoader assemblyLoader, IInterfaceProxyFactory proxyFactory, JsonHelper jsonHelper, ContentCoordinator contentCore, ModDatabase modDatabase, HashSet<string> suppressUpdateChecks, [NotNullWhen(false)] out ModFailReason? failReason, out string? errorReasonPhrase, out string? errorDetails)
        {
            ...
            else
            {
                ...
                catch (Exception ex)
                {
                    ...
                    failReason = ModFailReason.LoadFailed;
                    Monitor.Log("Catch try to load mod entry: " + ex.Message);
                    return false;
                }
            }
        }
        ...
        private bool TryLoadModEntry(IModMetadata metadata, Assembly modAssembly, out Mod? mod, out string? error)
        {
            ...
            {
                Monitor.Log("Erryr on use to array: " + ex.Message);
                Monitor.Log("Erryr on use to array: " + ex.InnerException);
                Monitor.Log("Erryr on use to array: " + ex.StackTrace);
            }
        ...
        *********/
        ...
        private readonly struct QueuedCommand : IEquatable<QueuedCommand>
        {
            public readonly Command Command;
            public readonly string Name;
            public readonly string[] Args;
            public QueuedCommand(Command Command, string Name, string[] Args)
            {
            ...
            public void Deconstruct(out Command command, out string name, out string[] args)
            {
            ...
            bool IEquatable<QueuedCommand>.Equals(QueuedCommand other)
            {
