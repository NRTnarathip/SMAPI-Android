        ...
        public IEnumerable<IAssetName> InvalidateCache(Func<IContentManager, string, Type, bool> predicate, bool dispose = false)
        {
            ...
            this.ContentManagerLock.InReadLock(() =>
            {
                ...
                foreach (IContentManager contentManager in this.ContentManagers)
                {
                    ...
                    //fix we just clone, Not reference 
                    //becuase you will got bug if you edit element within IEnumerable
                    var cachedAssets = contentManager.GetCachedAssets().ToArray();
                    foreach ((string key, object asset) in cachedAssets)
                    {
                        if (!predicate(contentManager, key, asset.GetType()))
                            continue;

                        AssetName assetName = this.ParseAssetName(key, allowLocales: true);
                        //Bug!! because you are try to Edit Element in this Loop with ref cachedAssets Enumerable
                        //In protected readonly ContentCache Cache;
