...
namespace Nanoray.Pintail
{
    internal static class TypeUtilities
    {
        ...
        private static int CompareTwoMethods(MethodInfo methodA, MethodInfo methodB)
        {
            ...
            var paramsAB = paramsA.Select((param, index) =>
                    new { ParamA = param, ParamB = paramsB.ElementAtOrDefault(index) });

            foreach (var paramItem in paramsAB)
            {
                var paramA = paramItem.ParamA;
                var paramB = paramItem.ParamB;
